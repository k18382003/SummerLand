...
import { CategoryOption } from '../../../app/common/form/dropdownopt/CategoryOption';
  ...
  export default observer(function ArticleForm()
  {
    ...
    function handleFormSubmit(article:Article){
      ...
      {
        article.artID = uuid();
        CreateArticle(article).then(()=>navigate(`/articles/${article.artID}`))
      }
      else
      {
        EditArticle(article).then(()=>navigate(`/articles/${article.artID}`))
      }
    }
          ...
          {({handleSubmit}) => (
              <Segment clearing>
                  <Form className='ui form' onSubmit={handleSubmit} autoComplete='off'>
                    ...
                    <Button as={Link} to="/articles" floated='right' color='grey' content='Cancel'/>
